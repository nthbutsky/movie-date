...
export const Modal = ({ isOpen, onClose, children }: TProps) => {
  return (
    <>
      {isOpen && (
        <>
          <div className="absolute left-0 top-0 z-50 h-full w-full bg-zinc-50 opacity-50 transition-all ease-in-out dark:bg-zinc-950"></div>
          <div className="group/main container absolute left-1/2 top-1/2 z-50 w-[calc(100%-32px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-md bg-zinc-50 shadow-glow shadow-violet-600 dark:bg-zinc-950 sm:w-full lg:max-w-[720px]">
            <button
              type="button"
